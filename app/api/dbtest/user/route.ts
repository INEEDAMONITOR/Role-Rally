import { dbConnect, validateEmail } from "@/app/api/utils";
import { Types } from "mongoose";
import UserModel from "@/app/api/models/User";
import { NextResponse } from "next/server";
import { createUser, deleteUser } from "../../controller/userController";
import { deleteProfile } from "../../controller/profileController";
...
export async function POST(request: Request) {
  ...
  if (!email || typeof email !== "string" || !validateEmail(email)) {
    return NextResponse.json({ message: "Invalid email",error: "Invalid email" }, { status: 400 });
  }
  ...
  try {
    ...
    if (newUser) {
      return NextResponse.json({ message: "Created successful", payload: newUser });
    } else {
      return NextResponse.json({ message: "Create fail. Email may already registered" }, { status: 400 });
    }
  } catch (error) {
    console.error(error);
