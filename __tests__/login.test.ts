...
describe("/api/login", () => {
  ...
  test("login/emptyEmail", async () => {
    const response = await loginHandler(
      createMockRequest({ password: "testpassword" })
    );
    expect(response.status).toBe(401);
    expect(await response.json()).toEqual({
      message: "Email can not be undefined",
  ...
  test("login/emptyPassword", async () => {
    const response = await loginHandler(
      createMockRequest({ email: "test@qq.com" })
    );
    expect(response.status).toBe(401);
    expect(await response.json()).toEqual({
  ...
  test("login/successLogin", async () => {
    const response = await loginHandler(
      createMockRequest({ email: "test1@qq.com", password: "test" })
    );
  ...
  test("login/failedToParseJSON", async () => {
    const response = await loginHandler(
      createMockRequest(new Error("Error 1"))
    );
    expect(response.status).toBe(401);
    expect(await response.json()).toEqual({
